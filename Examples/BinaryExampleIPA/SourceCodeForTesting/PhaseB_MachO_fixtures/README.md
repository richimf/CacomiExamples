...
These files are consumed by Cacomi's Mach-O binary-protection analyser as **standalone files** — point Cacomi at this folder (or at the individual `.dylib` files) to exercise all four M7 signals.

> ⚠️ **Do NOT place these dylibs inside the `BadPracticesIPA` app target / bundle.**
> They are intentionally minimal (~100-byte) synthetic Mach-O headers and are **not valid signable code**. Because the Xcode project uses *file-system synchronized groups*, any Mach-O dropped under `BadPracticesIPA/BadPracticesIPA/` is bundled into `Payload/BadPracticesIPA.app/` and Xcode will try to `codesign` it during IPA export, failing with:
> `codesign command failed ... internal error in Code Signing subsystem`.
> Keep them here, outside the app target, and scan them as loose files instead.
